...
% epoch block params (orientation, bar width, eccentricity, and green primary).
%
% Analysis note:
...
classdef DefocusBars < manookinlab.protocols.ManookinLabStageProtocol
    properties
        ...
        orientation = 'vertical'                            % Orientation of the bars
        barWidth = 150                                      % Bar width in microns (150 or 100)
    ...
    methods
        ...
        function prepareEpoch(obj, epoch)
            ...
            if iscell(matData)
                ...
                error('DefocusBars protocol requires that the images be organized into a cell array')
            end
        ...
        function fileFolder = get.fileFolder(obj)
            if obj.includeLCA
                if obj.invertLCA
                    fileFolder = sprintf('DefocusBars_%s_%dum_%decc_%dnm_invertedLCA', obj.orientation, obj.barWidth, obj.eccentricity, obj.greenPrimary);
                else
                    fileFolder = sprintf('DefocusBars_%s_%dum_%decc_%dnm', obj.orientation, obj.barWidth, obj.eccentricity, obj.greenPrimary);
                end
            else
                fileFolder = sprintf('DefocusBars_%s_%dum_%decc_%dnm_noLCA', obj.orientation, obj.barWidth, obj.eccentricity, obj.greenPrimary);
            end
        end
