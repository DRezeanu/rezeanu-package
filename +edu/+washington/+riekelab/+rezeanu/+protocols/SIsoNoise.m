classdef SIsoNoise < manookinlab.protocols.ManookinLabStageProtocol
    properties
        ...
        contrast = 1
        stixelSizes = [90,90, 150, 150]     % Edge length of stixel (microns)
        ...
        useCustomWeights = true             % Use custom color weights for S iso noise
        customWeights = [0.06, -0.08, 1]    % Custom color weights, only used if useCustomWeights is True
    end
    ...
    methods
        ...
        function prepareRun_regen(obj, canvas, run_params)
            obj.device_name = run_params.device_name;
            obj.canvasSize = canvas.size;
            obj.frameRate = run_params.frameRate;
            ...
            if obj.gaussianFilter
                % Get the gamma ramps.
                [r,g,b] = canvas.window.monitor.getGammaRamp();
                ...
                canvas.window.monitor.setGammaRamp(new_gamma, new_gamma, new_gamma);
            end             
        end
        ...
        function prepareRun(obj)
            ...
            obj.device_name = obj.rig.getDevice('Stage').name;
...
%                 @(state)state.frame > obj.pre_frames && state.frame < (obj.pre_frames + obj.unique_frames + obj.repeat_frames));
        ...
        function prepareEpoch_regen(obj, epoch_params)
            ...
            if obj.useCustomWeights
                obj.colorWeights = obj.customWeights;
            else
                obj.setColorWeights();
            end
            
            % Get the current stixel size.
            obj.stixelSize = obj.stixelSizes(mod(epoch_params.epoch-1, length(obj.stixelSizes))+1);
            obj.frameDwell = obj.frameDwells(mod(epoch_params.epoch-1, length(obj.frameDwells))+1);
            
            % Deal with the seed.
            obj.start_seed = epoch_params.seed;
            obj.seed = epoch_params.seed;
            
            obj.stepsPerStixel = max(round(obj.stixelSize / obj.gridSize), 1);
            
            gridSizePix = ceil(obj.gridSize/epoch_params.micronsPerPixel);
